...
def rm_gen(config):
    cont_out_f = config.gen_file_cont
    cont_fb_out_f = config.gen_file_cont_fb
    try:
        os.remove(cont_out_f)
    except OSError:
        pass
    try:
        os.remove(cont_fb_out_f)
    except OSError:
        pass


def init_nav(vehicle_ns):
    init_globals()
    config = car_config_ns.CarConfigNS(vehicle_ns)
...
def parse_path(config):
    ...
    speed_profile = []
    try:
        course_file = open(config.gen_file_course, 'r')
...
def logger(config, commands, feedback):
    commands = [str(commands[0]), str(commands[1]), str(commands[2])]
    feedback = [str(feedback[0]), str(feedback[1])]
    cmd_log_f = config.gen_file_cont
    fb_log_f = config.gen_file_cont_fb
    cmd_log = open(cmd_log_f, "a")
    for i in range(len(commands)):
        cmd_log.write(commands[i])
        cmd_log.write("\n")
    cmd_log.write("-------------------------\n")
    cmd_log.close()
    fb_log = open(fb_log_f, "a")
...
def exec_nav(config, path, pub_obj):
...
def main():
    while True:
        ...
        vehicle_ns = 'blue'
        config, path, pub_obj = init_nav(vehicle_ns)
        exec_nav(config, path, pub_obj)
        print('Done.')
        return

...
def init_ros(config):
    try:
        ...
        return pub_obj_thrtl, pub_obj_steer, pub_obj_brake
...
def send_init_cmd(config, pub_obj_thrtl, pub_obj_steer, pub_obj_brake):
    thrtl_pub = pub_obj_thrtl
    steer_pub = pub_obj_steer
    brake_pub = pub_obj_brake
    thrtl_init_msg = config.thrtl_msg_t
    steer_init_msg = config.steer_msg_t
    brake_init_msg = config.brake_msg_t
    thrtl_init_msg = 0.0
    steer_init_msg = 0.0
    brake_init_msg = 0.0
    thrtl_pub.publish(thrtl_init_msg)
    steer_pub.publish(steer_init_msg)
    brake_pub.publish(brake_init_msg)
...
def direct_publish(cmd_throttle, cmd_steer, cmd_brake):
    thrtl_msg = Float64()
    steer_msg = Float64()
    brake_msg = Float64()
    thrtl_msg.data = cmd_throttle
    steer_msg.data = cmd_steer / 1.22
    brake_msg.data = cmd_brake
    return thrtl_msg, steer_msg, brake_msg
...
def feedback_callback(odom_data):
    ...
    steer_pub.publish(Float64(cmd_steer * 1.57))
...
def main():
    while True:
        ...
        config, path, pub_obj_thrtl, pub_obj_steer, pub_obj_brake = init_nav()
        exec_nav(config, path, pub_obj_thrtl, pub_obj_steer, pub_obj_brake)
        print('Done.')
        return
